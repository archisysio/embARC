...
public class DPXReportService {
	...
	public static void WriteImageHashCsv(TableView<FileInformationViewModel> table, String csvFile) throws IOException {
		final FileWriter writer = new FileWriter(csvFile);
		...
		for (final FileInformationViewModel fivm : table.getItems()) {
			if (fivm == null) {
				continue;
			}
	...
	public static void WriteValidationCsv(TableView<FileInformationViewModel> table, String csvFile)
			throws IOException {
		...
		for (final FileInformationViewModel fivm : table.getItems()) {
			...
			final HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>> invalidRules = fivm
					.getInvalidRuleSets();

			// If we have any, iterate through each column, ruleset, and rule...writing each
			// to the CSV
			if (invalidRules.size() > 0) {
				for (final DPXColumn column : invalidRules.keySet()) {
