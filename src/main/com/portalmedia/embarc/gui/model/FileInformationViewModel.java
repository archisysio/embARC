 ...
 * @since 2019-05-01
 **/
public class FileInformationViewModel implements Mappable {

	@Id
	private int id;
	private boolean edited = false;
	private LinkedHashMap<String, String> properties;
	private HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>> invalidRules;
	private HashSet<ValidationRuleSetEnum> invalidRuleSetList;
	HashSet<IFileValidationRule> invalidFileRules;

	private HashMap<ValidationRuleSetEnum, List<DPXColumn>> ruleSetsBroken;
	...
	public FileInformationViewModel(String name, String path, String uuid) {
		super();
	}
	...
	public HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>> getInvalidRules() {
		return invalidRules;
	}

	public HashSet<ValidationRuleSetEnum> getInvalidRuleSetList() {
		invalidRuleSetList.clear();
		for (final DPXColumn c : invalidRules.keySet()) {
	...
	public HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>> getInvalidRuleSets() {
		return invalidRules;
	}

	public HashMap<ValidationRuleSetEnum, List<IValidationRule>> getInvalidRuleSets(DPXColumn column) {
	...
	public String getProp(DPXColumn column) {
	...
	public HashMap<ValidationRuleSetEnum, List<DPXColumn>> getRuleSetsBroken() {
		return ruleSetsBroken;
	}

	public HashMap<ValidationRuleSetEnum, List<IValidationRule>> getRuleViolations(DPXColumn column) {
		return invalidRules.get(column);
	}
	...
	public void read(NitriteMapper mapper, Document document) {
		if (document != null) {
			...
			setInvalidRules((HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>>) document
					.get("invalidRules"));
			...
			setRuleSetsBroken((HashMap<ValidationRuleSetEnum, List<DPXColumn>>) document.get("ruleSetsBroken"));
		}
	}
	...
	public void setInvalidRules(
			HashMap<DPXColumn, HashMap<ValidationRuleSetEnum, List<IValidationRule>>> invalidRules) {
	...
	public void setProp(DPXColumn column, String value) {
		String columnName = column.getDisplayName() + column.getSectionDisplayName();
		if (column.hasSubsection()) {
			columnName += column.getSubsection().getDisplayName();
		}
		...
		} else if (column == DPXColumn.USER_IDENTIFICATION) {
		...
		for (final ValidationRuleSetEnum ruleSet : ruleSets.keySet()) {
			...
			List<DPXColumn> brokenForThisRule = ruleSetsBroken.get(ruleSet);

			if (brokenForThisRule == null) {
				brokenForThisRule = new LinkedList<>();
			}
			brokenForThisRule.remove(column);
		...
		invalidRules.remove(column);
		if (!invalidRuleSets.isEmpty()) {
			invalidRules.put(column, invalidRuleSets);
		}
		setRuleSetErrors();
	}
	...
	private void setRuleSetErrors() {
		for (final ValidationRuleSetEnum rule : ValidationRuleSetEnum.values()) {
			int count = 0;
			final List<DPXColumn> columns = ruleSetsBroken.get(rule);
			if (columns != null) {
				count = columns.size();
			}
	...
	public void setRuleSetsBroken(HashMap<ValidationRuleSetEnum, List<DPXColumn>> ruleSetsBroken) {
